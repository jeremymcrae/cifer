...
plot_cnv <- function(z_scores, cnv) {
    ...
    pop_density = density(z_scores$population)
    ...
    plot(pop_density, xlab="L2R Z scores", ylab="Density", xlim=c(xmin, xmax),
        main=title, cex.main=0.8)
    
    # add the parental data points, so we can see how far away from the main
    # cluster they lie
    abline(v=z_scores$mom, col="blue", lty="dashed")
    abline(v=z_scores$dad, col="green", lty="dashed")
    abline(v=z_scores$proband, col="red", lty="dashed")
    legend("bottomleft", lty="dashed", legend=c("mom", "dad", "proband"),
